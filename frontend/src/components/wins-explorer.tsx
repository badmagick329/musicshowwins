  ...
  return (
    <div className="border border-border bg-card">
      <Table className="desktop-table border-collapse">
        ...
        <TableHeader><TableRow className="border-b-2 border-foreground bg-muted/50 text-xs uppercase tracking-[0.12em] text-muted-foreground"><TableHead className="w-32 px-4 py-3">Date</TableHead><TableHead className="px-4 py-3">Artist</TableHead><TableHead className="px-4 py-3">Song</TableHead><TableHead className="w-44 px-4 py-3 text-right">Music show</TableHead><TableHead className="w-44 px-4 py-3 text-right">Video</TableHead></TableRow></TableHeader>
        <TableBody>{wins.map((win) => <DesktopWinVideoRow key={win.id} win={win} colSpan={5} videoCellClassName="w-44 px-4 py-4 text-right"><TableCell className="px-4 py-4"><time dateTime={win.date} className="font-heading text-sm font-bold tabular-nums text-muted-foreground">{formatDate(win.date)}</time></TableCell><TableCell className="px-4 py-4"><Link prefetch={false} href={`/artists/${win.song.artist.id}`} className="compact-link-target font-semibold">{win.song.artist.name}</Link></TableCell><TableCell className="px-4 py-4"><Link prefetch={false} href={`/songs/${win.song.id}`} className="compact-link-target font-medium">{win.song.title}</Link></TableCell><TableCell className="w-44 px-4 py-4 text-right"><Link prefetch={false} href={`/wins?show=${encodeURIComponent(win.show.slug)}#wins-results-title`} aria-label={`Filter wins by ${win.show.name}`} className="compact-link-target"><ShowBadge slug={win.show.slug} name={win.show.name} /></Link></TableCell></DesktopWinVideoRow>)}</TableBody>
      </Table>
      <div className="mobile-record flex-col">
        {wins.map((win) => <article key={win.id} className="grid gap-2 border-b border-border/70 px-4 py-4 last:border-b-0"><time dateTime={win.date} className="font-heading text-sm font-bold tabular-nums text-muted-foreground">{formatDate(win.date)}</time><Link prefetch={false} href={`/artists/${win.song.artist.id}`} className="compact-link-target min-w-0 font-semibold">{win.song.artist.name}</Link><Link prefetch={false} href={`/songs/${win.song.id}`} className="compact-link-target min-w-0 font-medium">{win.song.title}</Link><Link prefetch={false} href={`/wins?show=${encodeURIComponent(win.show.slug)}#wins-results-title`} aria-label={`Filter wins by ${win.show.name}`} className="compact-link-target"><ShowBadge slug={win.show.slug} name={win.show.name} /></Link><MobileWinVideoDisclosure win={win} /></article>)}
      </div>
    </div>
  );
}
