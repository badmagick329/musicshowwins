      ...
      <ul className="mobile-record flex-col divide-y divide-border">
        {artists.map((artist) => <li key={artist.id}>
          ...
          >
            ...
            <span className="text-sm text-muted-foreground">{artist.latest_win_date ? `Latest win ${formatDate(artist.latest_win_date)}` : "No win date recorded"}</span>
          </Link>
        </li>)}
      </ul>
    </div>
