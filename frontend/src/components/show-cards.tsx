    ...
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {shows.map((show) => (
        <article key={show.id} className={`show-${show.slug} relative flex min-w-0 cursor-pointer flex-col border-2 border-foreground bg-card p-5 shadow-[4px_4px_0_var(--show-color)] transition-colors hover:bg-accent`}>
          ...
          <Link href={`/wins?show=${encodeURIComponent(show.slug)}#wins-results-title`} className="mt-5 inline-flex min-h-10 items-center justify-center border border-foreground bg-highlight-yellow px-4 text-sm font-bold shadow-[2px_2px_0_var(--foreground)] after:absolute after:inset-0 focus-visible:after:outline-2 focus-visible:after:outline-offset-[-2px] focus-visible:after:outline-brand-pink">View wins</Link>
        </article>
      ))}
    </div>
  );
