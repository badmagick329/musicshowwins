  ...
  return (
    <div className="border-2 border-foreground bg-card">
      <Table className="desktop-table border-collapse">
        ...
        <TableBody>{songs.map((song) => <TableRow key={song.id} className="border-border/70 hover:bg-accent/60"><TableCell className="px-4 py-4"><Link href={`/songs/${song.id}`} className="font-heading text-lg font-bold underline-offset-4 hover:underline">{song.title}</Link></TableCell><TableCell className="px-4 py-4"><Link href={`/artists/${song.artist.id}`} className="underline-offset-4 hover:underline">{song.artist.name}</Link></TableCell><TableCell className="px-4 py-4 text-right font-bold tabular-nums">{song.total_wins}</TableCell><TableCell className="px-4 py-4 tabular-nums text-muted-foreground">{song.latest_win_date ? formatDate(song.latest_win_date) : "No dated wins"}</TableCell><TableCell className="px-4 py-4 text-right tabular-nums">{song.winning_shows}</TableCell></TableRow>)}</TableBody>
      </Table>
      <div className="mobile-record flex-col">
      {songs.map((song) => <Link key={song.id} href={`/songs/${song.id}`} className="group grid gap-2 border-b border-border px-4 py-4 transition-colors last:border-b-0 hover:bg-accent focus-visible:bg-accent">
