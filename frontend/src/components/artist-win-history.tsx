...
export function ArtistWinHistory({ wins, emptyMessage = "No dated wins are recorded for this artist.", hideSong = false }: { wins: Win[]; emptyMessage?: string; hideSong?: boolean }) {
  ...
  return <div className="border border-border bg-card">
    <Table className="desktop-table border-collapse">
      <TableCaption className="sr-only">Dated music-show win history</TableCaption>
      ...
      <TableBody>{wins.map((win) => <TableRow key={win.id} className="border-border/70 hover:bg-accent/60"><TableCell className="px-4 py-3"><time dateTime={win.date} className="font-heading text-sm font-bold tabular-nums text-muted-foreground">{formatDate(win.date)}</time></TableCell>{!hideSong && <TableCell className="px-4 py-3"><Link href={`/songs/${win.song.id}`} className="font-semibold underline-offset-4 hover:underline">{win.song.title}</Link></TableCell>}<TableCell className="w-44 px-4 py-3 text-right"><ShowBadge slug={win.show.slug} name={win.show.name} /></TableCell></TableRow>)}</TableBody>
    </Table>
