...
export function SongsExplorer() {
  ...
  const apply = useCallback((update: Partial<SongFilters>) => write(update), [write]);
  ...
  return <main className="page-enter mx-auto max-w-7xl px-5 pb-8 pt-10 lg:px-8 lg:pt-14">
    ...
    <section className="mt-8" aria-labelledby="song-results-title"><div className="mb-4 flex flex-wrap items-end justify-between gap-3 border-b-2 border-foreground pb-3"><div><h2 id="song-results-title" className="font-heading text-2xl font-bold">Results</h2>{songs.isFetching && data && <p role="status" className="mt-1 text-xs text-muted-foreground">Updating results…</p>}</div>{data && <span className="text-sm tabular-nums text-muted-foreground">{data.count} {data.count === 1 ? "song" : "songs"}</span>}</div>
      ...
      {data && (data.previous || data.next) && <nav aria-label="Song pages" className="mt-6 flex items-center justify-between gap-4">{data.previous ? <button type="button" onClick={() => apply({ page: filters.page - 1 })} className="min-h-11 border-2 border-foreground bg-card px-4 text-sm font-bold shadow-[2px_2px_0_var(--foreground)]">Previous</button> : <span />}<span className="text-sm font-semibold tabular-nums">Page {filters.page}</span>{data.next ? <button type="button" onClick={() => apply({ page: filters.page + 1 })} className="min-h-11 border-2 border-foreground bg-card px-4 text-sm font-bold shadow-[2px_2px_0_var(--foreground)]">Next</button> : <span />}</nav>}
    </section>
  </main>;
}
